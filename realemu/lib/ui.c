// ui.c - handle the ui implementation for the emulator
#include <ui.h>
#include <emu.h>
#include <bus.h>
#include <ppu.h>
...
SDL_Window *sdlCheatWindow;
SDL_Renderer *sdlCheatRenderer;



static bool cheat_active= false;
...
void ui_init() {
    ...
    sdlCheatWindow = SDL_CreateWindow(
        "Cheat Console",                  // Window title
    ...
    sdlCheatRenderer = SDL_CreateRenderer(sdlCheatWindow, -1, 
                                        SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
    
    
    cheat_ui_init(sdlCheatRenderer);

    int x, y;
    SDL_GetWindowPosition(sdlWindow, &x, &y);
    SDL_SetWindowPosition(sdlDebugWindow, x + SCREEN_WIDTH + 10, y);
    SDL_SetWindowPosition(sdlCheatWindow, x  - 200, y);
...
void ui_update() {
    ...
    if (!emu_get_context()->paused) {
        ...
        if (cheat_ui_is_visible()) {
            cheat_ui_render();
            SDL_RenderPresent(sdlCheatRenderer);
        }


    }
    else {
        // When paused, just present the last frame again
        SDL_RenderPresent(sdlRenderer);
    }
...
void ui_handle_events() {
    ...
    while (SDL_PollEvent(&e) > 0) {
        ...
        cheat_ui_handle_event(&e); // pass event

        if (e.type == SDL_KEYDOWN && e.key.keysym.sym == SDLK_F1) {
            cheat_ui_toggle();
        }

        if (e.type == SDL_KEYDOWN && !cheat_ui_is_visible()) {
            ui_on_key(true, e.key.keysym.sym);

            if (e.key.keysym.sym == SDLK_p) {
                emu_get_context()->paused = !emu_get_context()->paused;
                printf("Paused: %s\n", emu_get_context()->paused ? "Yes" : "No");
